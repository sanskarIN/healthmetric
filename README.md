...
- Canonical newest-first history ordering across calculation, restore, and delete/undo flows.
- Collision-resistant UUID identifiers for new history entries.
...
- JSON restore with schema validation, bounded file size, bounded history, malformed-record recovery, duplicate-ID protection, and chronological normalization.
...
- About screen with explicit in-app/system-back navigation, license, version, GitHub, support contacts, funding link, and project credit.
...
- Automated Android emulator release-evidence capture for the required screenshot set.
- CI/release packaging for debug APK, unsigned release APK, and unsigned release Android App Bundle (AAB).
...
The Android instrumentation suite now includes a deterministic real-app release-evidence journey. On the API 35 Pixel 7 emulator used by CI, it captures the required PNG set from the built Android app and the workflow uploads those files as the `android-release-screenshots` artifact.

Automated captures:

1. Adult-use onboarding notice — `01-onboarding.png`.
2. Metric BMI calculator — `02-bmi-metric.png`.
3. Calculated BMI result using fictional/example data — `03-bmi-result.png`.
4. Waist-to-height calculator result using fictional/example data — `04-waist-ratio.png`.
5. Local history with the accessible chart — `05-history.png`.
6. Privacy/data settings — `06-settings.png`.
7. About and support screen — `07-about.png`.
8. Representative dark-theme settings screen — `08-dark-theme.png`.

Capture requirements and publication guidance are tracked in [`docs/assets/screenshots/README.md`](docs/assets/screenshots/README.md). CI-generated evidence is suitable for regression/release review; maintainers can select approved images for permanent README/store publication after visual and privacy review.
...
Run the local quality suite through the repository script:
...
Pull requests run dedicated workflows for the standard quality suite, Android emulator instrumentation and screenshot evidence, Apple shared-core compilation, CodeQL, dependency review, and full-history secret scanning.
...
The history retention setting caps stored results at 50, 100, 250, or 500 entries. Lowering the limit immediately trims older stored entries. History is normalized newest-first, including imported valid records and entries restored through the snackbar **Undo** action.
...
Backup reads/writes are capped at 1 MiB. Restore accepts schema version 1, caps restored history, rejects invalid top-level schemas, ignores malformed history records, keeps only the first valid record for duplicate IDs, and orders accepted records newest-first.
...
Unsigned release APK and App Bundle:

```bash
gradle :shared:desktopTest :shared:ktlintCheck :androidApp:ktlintCheck :androidApp:testDebugUnitTest :androidApp:lintRelease :androidApp:assembleRelease :androidApp:bundleRelease
```

The unsigned APK is written under `androidApp/build/outputs/apk/release/`; the unsigned AAB is written under `androidApp/build/outputs/bundle/release/`.

Pushing a `v*` tag runs the release workflow, verifies the project, creates both unsigned Android artifacts, uploads them as workflow artifacts, and attaches both to the generated GitHub Release. Store/distribution signing must be performed through a protected signing process; signing keys must never be committed.
...
- CI includes CodeQL, dependency review, full-history secret scanning, emulator tests, release screenshot evidence, and Apple target compilation.
...
- accessible labels for per-entry deletion and About back navigation;
...
See [`ROADMAP.md`](ROADMAP.md) for remaining physical-device, manual accessibility, signing, and initial-release work.
